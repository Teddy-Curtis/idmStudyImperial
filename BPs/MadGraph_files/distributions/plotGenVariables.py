...
def H_PT(H_data, dir): 
    ...
    plt.ylabel(f'Counts')
    #plt.legend()
    plt.savefig(f'{dir}/H_PT.jpeg', bbox_inches='tight')
    plt.close()
...
def A_PT(A_data, dir):
    ...
    plt.hist(A_data['Particle.PT'], bins=nbins)
    plt.title(f'A PT')
    plt.xlabel(f'PT (GeV)')
    plt.ylabel(f'Counts')
    #plt.legend()
...
def HA_PT_vectorSum(data, dir):
    ...
    plt.hist(inert_data['PT_tot'], bins=nbins)
...
def jet_PT(jet_data, dir):
    ...
    plt.hist(jet_data['Particle.PT'], bins=nbins)
    plt.title(f'Jet PT')
    plt.xlabel(f'PT (GeV)')
    plt.ylabel(f'Counts')
    #plt.legend()
