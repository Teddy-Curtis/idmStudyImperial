...
def plotDists(BP_params, lams, mass_splits, procs, save_loc):
    ...
    for var in var_names:
        ...
        for lam in lams:
            ...
            mass_splits = [1, 40, 80]
            for split in mass_splits:
                if not (((split==80) & (lam==0)) | ((split==1) & (lam==2))):
                    continue
                mHch = mA + split
...
plotDists(BP_params, lams, mass_splits, ["h2h2lPlM"], "no_cuts_slim/lPlM_allmHch_all_lam")

plotDists(BP_params, lams, mass_splits, ["h2h2lPlMnunu"], "no_cuts_slim/lPlMnunu_allmHch_all_lam")

plotDists(BP_params, lams, mass_splits, ["h2h2lPlM", "h2h2lPlMnunu"], "no_cuts_slim/lPlM_lPlMnunu_allmHch_all_lam")
