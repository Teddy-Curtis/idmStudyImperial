...
output_dir = f'/vols/cms/emc21/idmStudy/BPs/MadGraph_files/paramScan/trilepton/lam345scan/{process_name}'
...
for BP, params in BP_params.items():
    ...
    for lam in lams:
        with open(f'{output_dir}/MG_script_{BP}_lam345_{lam}.sh', 'w') as f:
            f.write('import model InertDoublet_UFO \n')
            for defintion in definitions:
                f.write(f'{defintion} \n')
            f.write(f'generate {process} \n')
            f.write(f'output {output_dir}/{process_name}_{BP}_lam345_{lam} \n')
                
            f.write(f'launch {output_dir}/{process_name}_{BP}_lam345_{lam} \n')
            ...
            f.write(f'launch {output_dir}/{process_name}_{BP}_lam345_{lam} -i \n')
            f.write(f'print_results --path={output_dir}/{process_name}_{BP}_lam345_{lam}/cross_section.txt --format=short \n')
