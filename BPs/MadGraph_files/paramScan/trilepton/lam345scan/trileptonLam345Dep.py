...
def plotDists(BPs, lams, procs, save_loc):
    ...
    for BP in BPs:
        ...
        for lam in lams:
            ...
            variables[f"{BP}_{lam}"] = [vars_all, xs]
    ...
    for var in var_names:
        ...
        for BP in BPs:
            ...
            for lam in lams:
                if var == "MET":
                    weights = variables[f"{BP}_{lam}"][0]['METweights'][0]
                else:
                    weights = variables[f"{BP}_{lam}"][0]['weights'][0]
                
                _ = plt.hist(variables[f"{BP}_{lam}"][0][var], bins=60, histtype='step', 
                    alpha=0.8, range=(settings['min'], settings['max']), 
                    label=f"lam345 = {lam}, xs={variables[f'{BP}_{lam}'][1]:.4f}pb", weights=weights)
...
plotDists(BPs, lams, ["h2h2lllnu"], "BP2/uncut")
