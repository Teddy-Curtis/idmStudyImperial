...
def getXS(proc, BP, split):
    file = f"{proc}/{proc}_BP{BP}_mA_split_{split}/cross_section.txt"
    with open(file) as f:
        lines = f.readlines()
        for line in lines:
...
def plotDists(BPs, splits, procs, save_loc):
    ...
    for BP in BPs:
        ...
        for split in splits:
            ...
            weights_all = []
            xs = 0
            filenames = [f"{proc}/{proc}_BP{BP}_mA_split_{split}/Events/run_01/unweighted_events.root:LHEF;1" for proc in procs]
        ...
        for BP in BPs:
            ...
            for split in splits:
                ...
                _ = plt.hist(variables[f"{BP}_{split}"][0][var], bins=60, histtype='step', 
                    ...
                    label=f"mA = mH + {split}, xs={variables[f'{BP}_{split}'][1]:.4f}pb", weights=weights)
