...
NEVENTS=500
OUTPATH=root://eosuser.cern.ch//eos/user/e/ecurtis/idmFilesEOS/gridpacks/$(tag)_parameterscan_updatedWidths_$(CMSSW_version)/$(run_name)
                               
# arguments 
GRIDPACK=root://eosuser.cern.ch//eos/user/e/ecurtis/idmFilesEOS/gridpacks/$(tag)_parameterscan_updatedWidths_$(CMSSW_version)/$(run_name)/$(run_name)_$(scram_version)_$(CMSSW_version)_tarball.tar.xz
...
#output = $(home)/logging/MCprod_$(run_name)_$(ClusterId).out
error = $(home)/logging/MCprod_$(run_name)_$(ClusterId).err
...
queue mH, mA from /afs/cern.ch/user/e/ecurtis/idmStudyImperial/MCproduction/genproductions/bin/MadGraph5_aMCatNLO/cards/iDMParameterScanUpdatedWidths_mHmA_noLLLNu/cards/new_inputs.txt
