...
NEVENTS=500
#! CHANGE THIS DEPENDING WHAT THE OUT FOLDER IS!!!
OUTPATH=root://eosuser.cern.ch//eos/user/e/ecurtis/idmFilesEOS/gridpacks/$(tag)/$(run_name)

# arguments 
#! CHANGE THIS DEPENDING WHAT THE OUT FOLDER IS!!!
GRIDPACK=root://eosuser.cern.ch//eos/user/e/ecurtis/idmFilesEOS/gridpacks/$(tag)/$(run_name)/$(run_name)_$(scram_version)_$(CMSSW_version)_tarball.tar.xz
...
output = $(home)/logging/test_$(tag)_$(run_name)_$(ClusterId).out
error = $(home)/logging/test_$(tag)_$(run_name)_$(ClusterId).err
#log = $(home)/logging/test_$(tag)_$(run_name)_$(ClusterId).log
...
# for i in {1..50}; do condor_submit condor_production.sh; done

#! CHANGE THIS DEPENDING ON THE MASS OF H!!!
queue mH, mA, mHch from /afs/cern.ch/user/e/ecurtis/idmStudyImperial/MCproduction/genproductions/bin/MadGraph5_aMCatNLO/cards/iDM_taus/mH80/input_arguments.txt